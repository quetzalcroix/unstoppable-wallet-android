package io.vextabit.bankwallet.modules.coin.adapters
...
class ChartViewHolder(override val containerView: View, private val listener: CoinChartAdapter.Listener, private val currency: Currency)
    ...
    private fun setSelectedPoint(item: ChartPointViewItem) {
        ...
        pointInfoPrice.text = io.vextabit.bankwallet.core.App.numberFormatter.formatFiat(item.price.value, item.price.currency.symbol, 2, 4)
        ...
        item.macdInfo?.let { macdInfo ->
            macdInfo.histogram?.let {
                ...
                macdHistogram.text = io.vextabit.bankwallet.core.App.numberFormatter.format(it, 0, 2)
            }
            macdInfo.signal?.let {
                macdSignal.isVisible = true
                macdSignal.text = io.vextabit.bankwallet.core.App.numberFormatter.format(it, 0, 2)
            }
            macdInfo.macd?.let {
                macdValue.isVisible = true
                macdValue.text = io.vextabit.bankwallet.core.App.numberFormatter.format(it, 0, 2)
            }
        }
    }

    private fun formatFiatShortened(value: BigDecimal, symbol: String): String {
        val shortCapValue = io.vextabit.bankwallet.core.App.numberFormatter.shortenValue(value)
        return io.vextabit.bankwallet.core.App.numberFormatter.formatFiat(shortCapValue.first, symbol, 0, 2) + " " + shortCapValue.second
    }
