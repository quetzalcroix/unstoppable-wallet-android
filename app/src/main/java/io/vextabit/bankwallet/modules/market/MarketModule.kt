...
object MarketModule {
    ...
    class Factory : ViewModelProvider.Factory {
        ...
        override fun <T : ViewModel?> create(modelClass: Class<T>): T {
            val service = MarketService(io.vextabit.bankwallet.core.App.marketStorage)
...
data class MarketViewItem(
    ...
    companion object {
        fun create(marketItem: MarketItem, marketField: MarketField): MarketViewItem {
            val formattedRate = io.vextabit.bankwallet.core.App.numberFormatter.formatFiat(marketItem.rate.value, marketItem.rate.currency.symbol, 0, 6)

            val marketDataValue = when (marketField) {
                MarketField.MarketCap -> {
                    val marketCapFormatted = marketItem.marketCap?.let { marketCap ->
                        val (shortenValue, suffix) = io.vextabit.bankwallet.core.App.numberFormatter.shortenValue(marketCap.value)
                        io.vextabit.bankwallet.core.App.numberFormatter.formatFiat(shortenValue, marketCap.currency.symbol, 0, 2) + " $suffix"
                    }

                    MarketDataValue.MarketCap(marketCapFormatted ?: Translator.getString(R.string.NotAvailable))
                }
                MarketField.Volume -> {
                    val (shortenValue, suffix) = io.vextabit.bankwallet.core.App.numberFormatter.shortenValue(marketItem.volume.value)
                    val volumeFormatted = io.vextabit.bankwallet.core.App.numberFormatter.formatFiat(shortenValue, marketItem.volume.currency.symbol, 0, 2) + " $suffix"
