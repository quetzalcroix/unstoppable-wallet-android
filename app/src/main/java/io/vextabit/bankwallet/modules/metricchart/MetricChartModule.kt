package io.vextabit.bankwallet.modules.metricchart
    ...
    class Factory(private val metricsChartType: MetricChartType?) : ViewModelProvider.Factory {
        ...
        override fun <T : ViewModel?> create(modelClass: Class<T>): T {
            val fetcher = when (val type = metricsChartType) {
                is MetricChartType.Coin -> CoinTvlFetcher(io.vextabit.bankwallet.core.App.xRateManager, type.coinType)
                is MetricChartType.TradingVolume -> CoinTradingVolumeFetcher(io.vextabit.bankwallet.core.App.xRateManager, type.coinType)
                is MetricChartType.MarketGlobal -> MarketGlobalFetcher(io.vextabit.bankwallet.core.App.xRateManager, type.metricsType)
                else -> throw IllegalArgumentException("Expected type of MetricChartType, but got different value")
            }

            val service = MetricChartService(io.vextabit.bankwallet.core.App.currencyManager.baseCurrency, fetcher)
            val factory = MetricChartFactory(io.vextabit.bankwallet.core.App.numberFormatter)
