package io.vextabit.bankwallet.modules.send.submodules.fee
...
object SendFeeModule {
    ...
    class Factory(
        ...
        override fun <T : ViewModel> create(modelClass: Class<T>): T {
            ...
            val feeCoinData = io.vextabit.bankwallet.core.App.feeCoinProvider.feeCoinData(coin)
            val feeCoin = feeCoinData?.first ?: coin

            val baseCurrency = io.vextabit.bankwallet.core.App.currencyManager.baseCurrency
            val helper = SendFeePresenterHelper(io.vextabit.bankwallet.core.App.numberFormatter, feeCoin, baseCurrency)
            val interactor = SendFeeInteractor(baseCurrency, io.vextabit.bankwallet.core.App.xRateManager, feeRateProvider, feeCoin)

            val presenter = SendFeePresenter(view, interactor, helper, coin, baseCurrency, feeCoinData, customPriorityUnit, FeeRateAdjustmentHelper(io.vextabit.bankwallet.core.App.appConfigProvider))
