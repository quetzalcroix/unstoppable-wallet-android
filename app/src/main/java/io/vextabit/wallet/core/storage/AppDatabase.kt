...
import io.vextabit.wallet.core.storage.migrations.Migration_31_32
import io.vextabit.wallet.core.storage.migrations.Migration_32_33
    ...
    companion object {
            ...
            override fun migrate(database: SupportSQLiteDatabase) {
                ...
                while (accountsCursor.moveToNext()) {
                    ...
                    if (id >= 0 && syncMode >= 0 && derivationColumnId >= 0) {
                        ...
                        val origin = when {
                            oldSyncMode.replaceFirstChar { it.lowercase(Locale.getDefault()) } == SyncMode.New.value.replaceFirstChar { it.lowercase(Locale.getDefault()) } -> AccountOrigin.Created.value
                ...
                while (walletsCursor.moveToNext()) {
                    ...
                    if (coinIdColumnIndex >= 0) {
                        ...
                        if (coinId == "BTC" || coinId == "BCH" || coinId == "DASH") {
                            ...
                            try {
                                walletSyncMode?.lowercase(Locale.getDefault())?.replaceFirstChar { if (it.isLowerCase()) it.titlecase(Locale.getDefault()) else it.toString() }?.let {
                            ...
                            if (derivationColumnIndex >= 0) {
                                ...
                                if (walletDerivation != null) {
                                    try {
                                        ...
                                        io.vextabit.wallet.core.App.localStorage.bitcoinDerivation = derivation
                                    } catch (e: Exception) {
        ...
        private val addBlockchainSettingsTable: Migration = object : Migration(14, 15) {
            override fun migrate(database: SupportSQLiteDatabase) {
                ...
                while (walletsCursor.moveToNext()) {
                    ...
                    if (coinIdColumnIndex >= 0) {
                        val coinId = walletsCursor.getString(coinIdColumnIndex)
                        val syncMode = io.vextabit.wallet.core.App.localStorage.syncMode ?: SyncMode.Fast
                        ...
                        when (coinId) {
                            "BTC" -> {
                                ...
                                derivationStr = (io.vextabit.wallet.core.App.localStorage.bitcoinDerivation
        ...
        private val MIGRATION_27_28: Migration = object : Migration(27, 28) {
            override fun migrate(database: SupportSQLiteDatabase) {
                ...
                io.vextabit.wallet.core.App.coinKit.saveCoins(customCoins)
                    ...
                    if (coinIdColumn >= 0) {
                        ...
                        if (changeColumn >= 0) {
                            ...
                            if (value != PriceAlert.ChangeState.OFF.value) {
                                val topic = "${coinId.uppercase(Locale.ENGLISH)}_24hour_${value}percent"
                        ...
                        if (trendColumn >= 0) {
                            ...
                            if (value != PriceAlert.TrendState.OFF.value) {
                                val topic = "${coinId.uppercase(Locale.ENGLISH)}_${value}term_trend_change"
        ...
        private fun updateCoinIdInEnabledWallets(customCoins: List<Coin>, database: SupportSQLiteDatabase) {
            val allCoins = io.vextabit.wallet.core.App.coinKit.getDefaultCoins() + customCoins
            val walletsCursor = database.query("SELECT * FROM EnabledWallet")
            while (walletsCursor.moveToNext()) {
                val coinIdColumnIndex = walletsCursor.getColumnIndex("coinId")
                var oldCoinId = ""
