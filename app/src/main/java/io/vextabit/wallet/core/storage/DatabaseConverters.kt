...
class DatabaseConverters {
    ...
    fun decryptSecretString(value: String?): SecretString? {
        ...
        return try {
            SecretString(io.vextabit.wallet.core.App.encryptionManager.decrypt(value))
        } catch (e: Exception) {
            null
        }
    }
    ...
    fun encryptSecretString(secretString: SecretString?): String? {
        return secretString?.value?.let { io.vextabit.wallet.core.App.encryptionManager.encrypt(it) }
    }
    ...
    fun decryptSecretList(value: String?): SecretList? {
        ...
        return try {
            SecretList(io.vextabit.wallet.core.App.encryptionManager.decrypt(value).split(","))
        } catch (e: Exception) {
            null
        }
    }
    ...
    fun encryptSecretList(secretList: SecretList?): String? {
        return secretList?.list?.joinToString(separator = ",")?.let {
            io.vextabit.wallet.core.App.encryptionManager.encrypt(it)
        }
    }
