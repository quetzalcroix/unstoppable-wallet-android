...
import io.vextabit.wallet.core.IAddTokenBlockchainService
...
class AddEvmTokenBlockchainService(
    ...
    override fun coinType(reference: String): CoinType {
        val address = reference.lowercase(Locale.ENGLISH)
        ...
        return networkManager.getEvmInfo(getApiUrl(networkType), request)
                .map { response ->
                    if (response.get("status").asString == "0") {
                        try {
                            if (response.get("result").asString.lowercase(Locale.ENGLISH).contains("limit reached")) {
