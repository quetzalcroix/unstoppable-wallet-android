...
class BalanceViewItemFactory {
    ...
    private fun coinValue(state: AdapterState?, balance: BigDecimal?, coin: Coin, visible: Boolean): DeemedValue {
        ...
        val value = balance?.let {
            val significantDecimal = io.vextabit.wallet.core.App.numberFormatter.getSignificantDecimalCoin(it)
            io.vextabit.wallet.core.App.numberFormatter.format(balance, 0, significantDecimal)
        }
    ...
    private fun currencyValue(state: AdapterState?, balance: BigDecimal, currency: Currency, latestRate: LatestRate?, visible: Boolean): DeemedValue {
        ...
        val value = latestRate?.rate?.let { rate ->
            io.vextabit.wallet.core.App.numberFormatter.formatFiat(balance * rate, currency.symbol, 0, 2)
        }
    ...
    private fun rateValue(currency: Currency, latestRate: LatestRate?, showSyncing: Boolean): DeemedValue {
        ...
        val value = latestRate?.let {
            ...
            io.vextabit.wallet.core.App.numberFormatter.formatFiat(latestRate.rate, currency.symbol, 2, 4)
        }
    ...
    private fun coinTypeLabel(wallet: Wallet) = when (wallet.coin.type) {
        CoinType.Bitcoin,
        CoinType.Litecoin -> {
            wallet.configuredCoin.settings.derivation?.value?.uppercase(Locale.ENGLISH)
        }
        CoinType.BitcoinCash -> {
            wallet.configuredCoin.settings.bitcoinCashCoinType?.value?.uppercase(Locale.ENGLISH)
        }
        else -> {
    ...
    private fun lockedCoinValue(state: AdapterState?, balance: BigDecimal, coinCode: String, hideBalance: Boolean): DeemedValue {
        ...
        val value = if (hideBalance) {
            ...
            val significantDecimal = io.vextabit.wallet.core.App.numberFormatter.getSignificantDecimalCoin(balance)
            io.vextabit.wallet.core.App.numberFormatter.formatCoin(balance, coinCode, 0, significantDecimal)
        }
    ...
    fun headerViewItem(items: List<BalanceModule.BalanceItem>, currency: Currency, balanceHidden: Boolean): BalanceHeaderViewItem = when {
        balanceHidden -> BalanceHeaderViewItem("*****", true)
            ...
            val balanceText = io.vextabit.wallet.core.App.numberFormatter.formatFiat(total, currency.symbol, 2, 2)
