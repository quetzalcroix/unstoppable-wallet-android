package io.vextabit.wallet.modules.coin.adapters
...
import io.vextabit.wallet.modules.coin.ChartInfoData
import io.vextabit.wallet.modules.coin.ChartPointViewItem
import io.vextabit.wallet.modules.coin.CoinViewModel
import io.vextabit.wallet.ui.extensions.createTextView
...
class ChartViewHolder(override val containerView: View, private val listener: CoinChartAdapter.Listener, private val currency: Currency)
    ...
    private fun setSelectedPoint(item: ChartPointViewItem) {
        ...
        pointInfoPrice.text = io.vextabit.wallet.core.App.numberFormatter.formatFiat(item.price.value, item.price.currency.symbol, 2, 4)
        ...
        item.macdInfo?.let { macdInfo ->
            macdInfo.histogram?.let {
                ...
                macdHistogram.text = io.vextabit.wallet.core.App.numberFormatter.format(it, 0, 2)
            }
            macdInfo.signal?.let {
                macdSignal.isVisible = true
                macdSignal.text = io.vextabit.wallet.core.App.numberFormatter.format(it, 0, 2)
            }
            macdInfo.macd?.let {
                macdValue.isVisible = true
                macdValue.text = io.vextabit.wallet.core.App.numberFormatter.format(it, 0, 2)
            }
        }
    }

    private fun formatFiatShortened(value: BigDecimal, symbol: String): String {
        val shortCapValue = io.vextabit.wallet.core.App.numberFormatter.shortenValue(value)
        return io.vextabit.wallet.core.App.numberFormatter.formatFiat(shortCapValue.first, symbol, 0, 2) + " " + shortCapValue.second
    }
