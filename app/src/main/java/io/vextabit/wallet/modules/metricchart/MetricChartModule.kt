...
import io.vextabit.wallet.modules.coin.metricchart.CoinTradingVolumeFetcher
import io.vextabit.wallet.modules.coin.metricchart.CoinTvlFetcher
    ...
    class Factory(private val metricsChartType: MetricChartType?) : ViewModelProvider.Factory {
        ...
        override fun <T : ViewModel?> create(modelClass: Class<T>): T {
            val fetcher = when (val type = metricsChartType) {
                is MetricChartType.Coin -> CoinTvlFetcher(io.vextabit.wallet.core.App.xRateManager, type.coinType)
                is MetricChartType.TradingVolume -> CoinTradingVolumeFetcher(io.vextabit.wallet.core.App.xRateManager, type.coinType)
                is MetricChartType.MarketGlobal -> MarketGlobalFetcher(io.vextabit.wallet.core.App.xRateManager, type.metricsType)
                else -> throw IllegalArgumentException("Expected type of MetricChartType, but got different value")
            }

            val service = MetricChartService(io.vextabit.wallet.core.App.currencyManager.baseCurrency, fetcher)
            val factory = MetricChartFactory(io.vextabit.wallet.core.App.numberFormatter)
