package io.vextabit.wallet.modules.send
...
import io.vextabit.wallet.modules.send.binance.SendBinanceHandler
import io.vextabit.wallet.modules.send.binance.SendBinanceInteractor
import io.vextabit.wallet.modules.send.bitcoin.SendBitcoinHandler
import io.vextabit.wallet.modules.send.bitcoin.SendBitcoinInteractor
import io.vextabit.wallet.modules.send.dash.SendDashHandler
import io.vextabit.wallet.modules.send.dash.SendDashInteractor
import io.vextabit.wallet.modules.send.ethereum.SendEthereumHandler
import io.vextabit.wallet.modules.send.ethereum.SendEthereumInteractor
import io.vextabit.wallet.modules.send.submodules.address.SendAddressModule
import io.vextabit.wallet.modules.send.submodules.amount.SendAmountModule
import io.vextabit.wallet.modules.send.submodules.fee.CustomPriorityUnit
...
import io.vextabit.wallet.modules.send.zcash.SendZcashHandler
import io.vextabit.wallet.modules.send.zcash.SendZcashInteractor
...
object SendModule {
    ...
    class Factory(private val wallet: Wallet) : ViewModelProvider.Factory {
        ...
        override fun <T : ViewModel> create(modelClass: Class<T>): T {
            ...
            val handler: ISendHandler = when (val adapter = io.vextabit.wallet.core.App.adapterManager.getAdapterForWallet(wallet)) {
                is ISendBitcoinAdapter -> {
                    val bitcoinInteractor = SendBitcoinInteractor(adapter, io.vextabit.wallet.core.App.localStorage)
    ...
    sealed class AmountInfo {
        ...
        fun getFormatted(): String = when (this) {
            ...
            is CurrencyValueInfo -> {
                io.vextabit.wallet.core.App.numberFormatter.formatFiat(currencyValue.value, currencyValue.currency.symbol, 2, 2)
            }
        }

        fun getFormattedPlain(): String = when (this) {
            is CoinValueInfo -> {
                io.vextabit.wallet.core.App.numberFormatter.format(value, 0, 8)
            }
            is CurrencyValueInfo -> {
                io.vextabit.wallet.core.App.numberFormatter.formatFiat(currencyValue.value, currencyValue.currency.symbol, 2, 2)
            }
        ...
        fun getFormattedForTxInfo(): String = when (this) {
            ...
            is CurrencyValueInfo -> {
                val significantDecimal = io.vextabit.wallet.core.App.numberFormatter.getSignificantDecimalFiat(currencyValue.value)

                io.vextabit.wallet.core.App.numberFormatter.formatFiat(currencyValue.value, currencyValue.currency.symbol, 0, significantDecimal)
            }
        }
